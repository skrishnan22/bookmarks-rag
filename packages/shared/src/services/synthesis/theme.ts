...
export const ThemeNameSchema = z.enum(["default", "warm", "cool", "mono", "vibrant"]);
...
export interface ThemeColors {
  ...
  mentalModelBg: string;
  mentalModelBorder: string;
  questionBg: string;
...
// Helper to generate CSS custom properties from a theme
export function themeToCssVars(theme: Theme): Record<string, string> {
  const vars: Record<string, string> = {};

  for (const [key, value] of Object.entries(theme.colors)) {
    vars[`--synth-color-${toKebabCase(key)}`] = value;
  }

  vars["--synth-font-family"] = theme.typography.fontFamily;
  vars["--synth-font-weight-heading"] = theme.typography.headingWeight;
  vars["--synth-font-weight-body"] = theme.typography.bodyWeight;
  vars["--synth-spacing-card"] = theme.spacing.cardPadding;
  vars["--synth-spacing-section"] = theme.spacing.sectionGap;
  vars["--synth-spacing-component"] = theme.spacing.componentGap;
  vars["--synth-border-radius"] = theme.borderRadius;
