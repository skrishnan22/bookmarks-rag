...
export const Route = createFileRoute("/entities/tv-shows")({
  component: TvShowsPage,
});

function TvShowsPage() {
  const [selectedEntity, setSelectedEntity] = useState<Entity | null>(null);
  const { data, isLoading } = useEntities("tv_show");
      ...
      >
        ...
        >
          <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-white border border-zinc-50 shadow-soft">
            <Tv className="h-8 w-8 text-zinc-300" />
          </div>
          <motion.div
            animate={{ y: [0, -8, 0] }}
        ...
        <h3 className="text-lg font-bold text-zinc-900">No TV shows found</h3>
        <p className="mt-1 text-sm text-zinc-500 font-medium">
          TV shows mentioned in your bookmarks will appear here
        </p>
      </motion.div>
    );
  }
